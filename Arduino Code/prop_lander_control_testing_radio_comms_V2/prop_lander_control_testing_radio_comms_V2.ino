...
float throttleOutput = 70; // Current throttle output
float throttleTarget = 0; // Target throttle from the PID
float maxSlewRate = 5; // Maximum change per control loop (e.g., 0.5% per iteration)

int desiredDerivativeRateMin = 80; // mm/s
int desiredDerivativeRateMax = 300; // mm/s
int previousOutput = 0;

int minThrottle = 70; // Minimum throttle
int maxThrottle = 100; // Maximum throttle

int velocityConstrainedMinThrottle = 0;
int velocityConstrainedMaxThrottle = 0;

float minHeight = 165;
float groundEffectHeight = 260;
...
int getDistance()
{
  ...
  if (status == 0 && newDataReady) { // If data is ready and no errors, get measurement data
    ...
    // BEFORE USING THIS CHUNK DISTANCE SENSOR NEEDS TO BE TESTED OUTDOORS
...
void setup() 
{
  ...
  Serial.println("LABEL,T,H,Const,PID,Slew,TMax,TMin,E,dE");
  sendRadioComms("LABEL,T,H,Const,PID,Slew,TMax,TMin,E,dE");
...
void loop() 
{
  ...
  if (currentTime - previousTime >= deltaTime * 1000) {
    ...
    if (derivative <= -desiredDerivativeRateMin && derivative >= -desiredDerivativeRateMax && currentHeight >= groundEffectHeight) {
      velocityConstrainedMaxThrottle = max(previousOutput, velocityConstrainedMaxThrottle);
      maxSlewRate = 5;
    }

    if (derivative >= 40 && derivative <= desiredDerivativeRateMax) {
      velocityConstrainedMinThrottle = previousOutput;
      maxSlewRate = 5;
    }

    if (derivative >= -desiredDerivativeRateMin && derivative <= desiredDerivativeRateMin && currentHeight <= minHeight) {
    ...
    if (derivative >= -desiredDerivativeRateMin && derivative <= desiredDerivativeRateMin && currentHeight >= groundEffectHeight) {
      maxSlewRate = 1;
    }
    ...
    String data = "DATA," + String(currentTime) + "," + String(currentHeight) + "," + String(constrainedOutput) + "," + String(pidOutput) + "," + String(slewRateLimitedOutput) + "," + String(velocityConstrainedMaxThrottle) + "," + String(velocityConstrainedMinThrottle) + "," + String(error) + "," + String(derivative);
