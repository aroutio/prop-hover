...
#define ESC1_PIN 9
#define ESC2_PIN 10
#define UPPER_GIMBAL_PIN 11
#define LOWER_GIMBAL_PIN 12

Adafruit_ZeroTimer zt(0); // Timer instance for PWM
...
int upperGimbalCenter = 1500;
int lowerGimbalCenter = 1500;

int pulseWidth = 0;

// Function to configure PWM for a pin
void configurePWM(uint8_t pin, uint16_t frequency, uint16_t minPulse, uint16_t maxPulse) {
  pinPeripheral(pin, PIO_TIMER_ALT); // Assign pin to timer
  zt.enable(true);
  zt.configure(TC_CLOCK_PRESCALER_DIV1024, TC_COUNTER_SIZE_16BIT, TC_WAVE_GENERATION_MATCH_PWM);
  zt.setPeriodMatch(frequency, 0); // Set frequency
  zt.PWMout(pin - 9, frequency, pulseWidth);
}

// Function to set PWM pulse width (microseconds)
void setPWM(uint8_t pin, uint16_t pulseWidth) {
  uint16_t compareValue = pulseWidth * 2; // Convert µs to timer compare value
  zt.PWMout(pin - 9, compareValue, pulseWidth); // Update PWM signal
}
...
void setThrottle1(int throttle)
{
  ...
  setPWM(ESC1_PIN, throttle);
}

void setThrottle2(int throttle)
{
  throttle = map(throttle, 0, 1000, 1000, 2000);
  setPWM(ESC2_PIN, throttle);
}

void sendRadioComms(String data) 
{
  // Convert String to char array
  char telemetry[data.length() + 1];
  data.toCharArray(telemetry, sizeof(telemetry));
  telemetry[data.length()] = '\0'; // Null-terminate

  // Send the data packet
  rf69.send((uint8_t *)telemetry, strlen(telemetry));

  // Non-blocking wait for packet transmission
  unsigned long startTime = millis();
  while (!rf69.waitPacketSent()) { // Check if packet has been sent
      delayMicroseconds(10);

      // Timeout to prevent infinite loop
      if (millis() - startTime > 100) {
          Serial.println("SPI transmission timeout!");
          return;
      }
  }
...
void setup() 
{
  ...
  configurePWM(ESC1_PIN, 50, 1000, 2000);
  configurePWM(ESC2_PIN, 50, 1000, 2000);
  setThrottle1(0); // Set throttle to 0%
  setThrottle2(0); // Set throttle to 0%
  Serial.println("Throttle LOW");
  sendRadioComms("Throttle LOW");

  configurePWM(UPPER_GIMBAL_PIN, 50, 1000, 2000);
  configurePWM(LOWER_GIMBAL_PIN, 50, 1000, 2000);
  setPWM(UPPER_GIMBAL_PIN, upperGimbalCenter * 10); // Convert to µs
  delay(15);
  setPWM(LOWER_GIMBAL_PIN, lowerGimbalCenter * 10);
  delay(15);
